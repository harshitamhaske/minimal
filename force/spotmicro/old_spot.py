...
class Spot(object):
  ...
  """
    ...
    def GetGRF(self):
        ...
        lower_leg_joints = [
        8,  # Front left lower
        12, # Front right lower
        17, # Back left lower
        21  # Back right lower
        ]

        foot_joint_indices = {
        'front_left_leg_foot': 9,
        'front_right_leg_foot': 13,
        'back_left_leg_foot': 18,
        'back_right_leg_foot': 22
         }
        ...
        for joint_index in range(self._pybullet_client.getNumJoints(self.quadruped)):
         if joint_index in hip_joints:
          hip_angles.append(self._pybullet_client.getJointState(self.quadruped, joint_index)[0])
         elif joint_index in upper_leg_joints:
          upper_leg_angles.append(self._pybullet_client.getJointState(self.quadruped, joint_index)[0])
         elif joint_index in lower_leg_joints:
          lower_leg_angles.append(self._pybullet_client.getJointState(self.quadruped, joint_index)[0])
        
        # Print the joint angles
        #print("Hip Angles:", hip_angles)
        #print("Upper Leg Angles:", upper_leg_angles)
        #print("Lower Leg Angles:", lower_leg_angles)
        ...
        for i in range(4):
         psi = hip_angles[i]
         theta = upper_leg_angles[i]
         phi = lower_leg_angles[i]

         lower_to_upper_leg_rot=np.array([
                                         [np.cos(phi),0,np.sin(phi)],
                                         [0, 1, 0],
                                         [-np.sin(phi),0,np.cos(phi)]
                                         ]) 
         upper_leg_to_hip_rot=np.array([
                                         [np.cos(theta),0,np.sin(theta)],
                                         [0, 1, 0],
                                         [-np.sin(theta),0,np.cos(theta)]
                                        ]) 
         hip_to_body_rot=np.array([
                                 [1,0,0],
                                 [0,np.cos(psi),-np.sin(psi)],
                                 [0,np.sin(psi),np.cos(psi)]
                                 ])
         
         foot_to_world_TR[foot_joint_list[i]]=lower_to_upper_leg_rot @ upper_leg_to_hip_rot @ hip_to_body_rot
        

        grf_in_joint_frame = {}

  
        for foot_joint_name, foot_joint_index in foot_joint_indices.items():
         force_torque = self._pybullet_client.getJointState(self.quadruped, foot_joint_index)[2]
         grf_in_joint_frame[foot_joint_name] = np.array(force_torque[:3])

        GRF = {}

   
        for foot_joint_name in foot_joint_indices:
         rotation_matrix = foot_to_world_TR[foot_joint_name]
         grf_vector = grf_in_joint_frame[foot_joint_name]
       
         grf_in_base_frame = rotation_matrix @ grf_vector
         GRF[foot_joint_name] = (grf_in_base_frame)


         GRF[foot_joint_name] = -1 * grf_in_base_frame
    ...
    def GetObservation(self):
        ...
        grf_forces = self.GetGRF()

        GRF_FL = grf_forces.get('front_left_leg_foot', [0, 0, 0])
        GRF_FR = grf_forces.get('front_right_leg_foot', [0, 0, 0])
        GRF_BL = grf_forces.get('back_left_leg_foot', [0, 0, 0])
        GRF_BR = grf_forces.get('back_right_leg_foot', [0, 0, 0])
       
     
       
        grf_values = np.concatenate([GRF_FL, GRF_FR, GRF_BL, GRF_BR])
